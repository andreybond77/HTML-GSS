// src/data.ts
// Сначала определяем интерфейсы для работы с валютами и товарами
export interface CurrencyPrices {
  shmeckles: number;    // Шмекели - основная валюта из вселенной Рика и Морти
  credits: number;      // Кредиты - межгалактическая валюта
  flurbos: number;      // Флурбо - валюта из измерения C-137
}
...
export interface Product {
  ...
  name: string;
  description: string;
  prices: CurrencyPrices;  // Теперь цены в разных валютах
  imageUrl: string;
}

// Типы валют для переключателя
